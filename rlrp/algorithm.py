...
def algorithm(params: AlgorithmParams, type: AlgorithmType = None, log_header = True):
    ...
    ret = None
    if params.app.inst.strings.APPLICATION_NAME == "lrp":
        # calculate optimal solution for each scenario's second stage problem and save the assignments of store to depot and the
        # recovered warehouse sizes to a object that we return
        ret = LRPReturnObject()
        for sc in S:
            second_stage_warmstart = None
            if sc in D:
                second_stage_warmstart = dm.get_second_stage_solution_for_scenario(sc)  # is None when sc not in D
            ssm = params.app.SecondStageModel(instance=params.app.inst, k=sc,
                                              first_stage_solution=first_stage,
                                              warmstart=second_stage_warmstart)
            # ssm.Params.Threads = params.n_threads
            # ssm.Params.BestObjStop = z_tilde
            ssm.optimize()
